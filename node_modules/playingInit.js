function playingInit(io, pool, teamplayGameField){
    ...
    function playerDisconnected(game, username) {
        ...
        if (!game.player1.isInGame && (!game.player2 || !game.player2.isInGame)) {
            gameOver(game.gameId);
        } else if (game.gameOver || game.finishTime) {
            io.sockets.in(`Game${game.gameId}`).emit("replayDeclined", {gameId: game.gameId});
        }
    }
    ...
    function connectionHandler(socket){
        ...
        socket.on("cell-opened", data => {
            ...
            if (game.gameOver || game.finishTime) return socket.emit("myError", {status: 404, text: "cell-opened: game is finished"});

            const {minesOnTheField, fieldInfo, player1, player2} = game; 

            if (fieldInfo[data.cellId] && fieldInfo[data.cellId]["cell-opened"]) return;
        ...
        socket.on("flag", data => {
            ...
            if (game.gameOver || game.finishTime) return socket.emit("myError", {status: 404, text: "flag: game is finished"});
        ...
        socket.on("playAgain", data => {
            ...
            if (game) {
                ...
                let clients = Object.keys(io.sockets.adapter.rooms[`Game${game.gameId}`].sockets);

                if (clients.length !== 2) return socket.emit("replayDeclined");          
    ...
    function openTheCell(targetNum, minesOnTheField, fieldInfo){
        ...
        while (queue.length) {
            ...
            if (fieldInfo[next] === undefined)
                return {};    
            //return socket.emit("myError", {status: 500, text: "Some weird error in OpenTheCell method ¯\\_(ツ)_/¯"});
    ...
    function rankChange(game) {
        ...
        let ret = {
            [game.player1.username]: player1PTS, 
            [game.player2.username]: player2PTS
        };

        let players = [game.player1.username, game.player2.username];

        ret.player1 = {
            username: game.player1.username,
            start: game.player1.isGuest ? undefined : game.player1.rank,
            delta: game.player1.isGuest ? undefined : ret[players[0]],
            total: game.player1.isGuest ? undefined : game.player1.rank + ret[players[0]],
            touches: game.player1.touches
        };
        ret.player2 = {
            username: game.player2.username,
            start: game.player2.isGuest ? undefined : game.player2.rank,
            delta: game.player2.isGuest ? undefined : ret[players[1]],
            total: game.player2.isGuest ? undefined : game.player2.rank + ret[players[1]],
    ...
    function updateRating(game) {
        ...
        if (game.player1.isGuest && game.player2.isGuest) {
        ...
        pool.connect((err, client, release) => {
            ...
            client.query(`update users set
                                rank = new_data.rank
                                from (values
                                    ${game.player1.isGuest ? "" : `('${calcedRankChange.player1.username}', ${calcedRankChange.player1.total})`}
                                    ${!game.player1.isGuest && !game.player2.isGuest ? "," : ""}
                                    ${game.player2.isGuest ? "" : `('${calcedRankChange.player2.username}', ${calcedRankChange.player2.total})`}
