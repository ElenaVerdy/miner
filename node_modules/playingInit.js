function playingInit(io, pool, teamplayGameField){
    ...
    function connectionHandler(socket){
        ...
        socket.on("getFieldInfo", data => {
            ...
            if (!game) return socket.emit("error", {status: 404});

            socket.emit("fieldUpdated", {fieldUpdate: game.fieldInfo});            
        });
        ...
        socket.on("join", (data) => {
            ...
            io.of('/').in(`Game${data.gameId}`).clients(function(error,clients){
                ...
                if (clients.length == 2 && !game.isGameOn) {
                    ...
                    io.in(`Game${data.gameId}`).emit("ready", {player1, player2, startTime: Date.now() + 2000});
        ...
        socket.on("gameStart", (data)=>{
            ...
            if (!game.player1 || !game.player2)
                return io.sockets.in(`Game${game.gameId}`).emit("error", {status: 500});
        ...
        socket.on("cell-opened", data => {
            ...
            if (minesOnTheField[data.cellId] === true) {
                ...
                pool.connect((err, client, release) => {
                    ...
                    if (err) {
                        console.log(err)
                        io.sockets.in(`Game${game.gameId}`).emit("error", {status: 500});
                    }
        ...
        socket.on("playAgain", data => {
            ...
            if (game) {
                ...
                io.of('/').in(`Game${game.gameId}`).clients(function(error, clients){
                    ...
                    if (error) return socket.emit("error", {status: 500});
                    if (clients.length !== 2) return socket.emit("error", {status: 444, text: "Your teammate already left."});
    ...
    function rankChange(game, loserUsername) {
        ...
        if (game.finishTime) {
            ...
            let PTSforLOSS = game.fieldInfo.filter(item => {
                return !item["cell-closed"]; 
            }).length / (width * height - num) * PTSforWIN / 2;

            if (game.player1.username == loserUsername) {
            
                player1PTS = -25;
                player2PTS = Math.round(PTSforLOSS * (1 - player1ratio)) - 20; 
            
            } else if (game.player2.username == loserUsername) {
            
                player1PTS = Math.round(PTSforLOSS * player1ratio) - 20; 
                player2PTS = -25;
    ...
    function gameWon(gameId) {
        ...
        pool.connect((err, client, release) => {
            ...
            if (err) {
                ...
                io.sockets.in(`Game${gameId}`).emit("error", {status: 500});
            }
            client.query(query, (err, result) => {
                ...
                pool.connect((err, client, release) => {
                    ...
                    client.query(query, (err, result) => {
                        ...
                        if (k === -1) {
                            io.sockets.in(`Game${gameId}`).emit("error", {status: 500, messege: "The game was not inserted into db!"});
                        } else {
